...
if ( $nv_Request->get_int( 'edit', 'post', 0 ) == 1 )
{
	...
	{	
		$query = $db->exec("UPDATE `" . NV_PREFIXLANG . "_" . $module_data . "` SET `".$key."` = " . $db->dbescape( $data ) . " WHERE `id` =" . $id . "");
	}
	if ( $query ) 
	{
...
{	
	// lay theo post
	$rowcontent['title'] = $nv_Request->get_title( 'title', 'post', '' );
	$rowcontent['content'] = $nv_Request->get_title( 'content', 'post', '' );
	$rowcontent['key'] = $nv_Request->get_title( 'key', 'post', '' );
	$rowcontent['quession'] = $nv_Request->get_title( 'quession', 'post', '' );
	...
	else
	{
		...
		( 
			NULL, 
			'" . $db->dblikeescape( $rowcontent['title'] ) . "', 
			'" . $db->dblikeescape( $rowcontent['content'] ) . "', 
			'" . $db->dblikeescape( $rowcontent['key'] ) . "' ,
			'" . $db->dblikeescape( $rowcontent['quession'] ) . "'
		)
		"; 
